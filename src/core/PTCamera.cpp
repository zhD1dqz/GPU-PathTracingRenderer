...
namespace PathTraceAlg
{
    ...
    {
        radius = std::max(radius + dr, 0.05f);
        UpdateCamera();
    }

    void Camera::Zoom(float wheelSteps)
    {
        radius = std::max(radius * powf(0.88f, wheelSteps), 0.05f);
        UpdateCamera();
    }

    void Camera::SetLookAt(const Vec3& eye, const Vec3& lookat)
    {
        position = eye;
        pivot = lookat;
        radius = Vec3::Distance(eye, lookat);
...
├── 景深效果：光圈和焦距控制
├── 透视投影：FOV视野控制
└── 矩阵计算：LookAt和Perspective

交互控制：
├── OffsetOrientation() - 旋转相机
├── Strafe()          - 平移相机
└── SetRadius()       - 缩放相机*/
